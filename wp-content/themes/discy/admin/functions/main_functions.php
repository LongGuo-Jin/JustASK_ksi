...
if (is_admin()) {
	...
	function discy_screen_belongs_to_theme_options() {
		...
		if ( ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'options' ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && ($_GET['page'] == 'r_questions' || $_GET['page'] == 'r_answers' || $_GET['page'] == 'wpqa_new_categories') ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'wpqa_payments' ) || in_array( $pagenow, $white_label ) || $show_admin_scripts == true ) {
			return true;
		}
		return false;
	}
	...
	function discy_enqueue_scripts( $hook ) {
		...
		if ( $enqueue ) {
			...
			wp_enqueue_script("options-custom",discy_framework_dir."js/options-custom.js",array("jquery","wp-color-picker","jquery-ui-sortable","jquery-ui-datepicker"),discy_theme_version);
			$discy_js = array(
				...
				"ask_question"              => esc_html__("Select ON to ask a question.","discy"),
				...
				"add_category"              => esc_html__("Select ON to add a category.","discy"),
				"send_message"              => esc_html__("Select ON to send message.","discy"),
				"upload_files"              => esc_html__("Select ON to can upload files.","discy"),
				"approve_question"          => esc_html__("Select ON to approve the questions auto.","discy"),
				"approve_answer"            => esc_html__("Select ON to approve the answers auto.","discy"),
				"approve_post"              => esc_html__("Select ON to approve the posts auto.","discy"),
				"approve_comment"           => esc_html__("Select ON to approve the comments auto.","discy"),
				"approve_question_media"    => esc_html__("Select ON to approve the questions auto when have media attached.","discy"),
				"approve_answer_media"      => esc_html__("Select ON to approve the answers auto when have media attached.","discy"),
			);
...
function discy_excerpt ($excerpt_length,$excerpt_type = discy_excerpt_type,$read_more = false,$return = "") {
	global $post;
...
function discy_comment($comment,$args,$depth,$answer = "",$owner = "",$k_ad = "",$best_answer = "",$answer_args = array()) {
    ...
    $like_answer = apply_filters("discy_comment_like_answer",false,(isset($post->post_type)?$post->post_type:""));
    if ($like_answer == true || ($answer != "answer" && isset($post->post_type) && $post->post_type == 'question') || ($answer == "answer" && $post_type == "question")) {
    	...
    	$the_best_answer = discy_post_meta("the_best_answer",($answer == "answer"?$post_id:(isset($post->ID)?$post->ID:0)),false);
                ...
                <div class="author clearfix">
                	...
                	<div class="comment-meta">
                    	<div class="comment-author">
                    		...
                    		if ($wpqa_activate_comment_author == true) {
                    			if ($comment_user_id > 0) {
	                    			$get_author_posts_url = get_author_posts_url($comment_user_id);
	                    		}else {
	                    		...
	                    		<?php }
	                        		...
	                        		echo ($deleted_user == "delete"?esc_html__("[Deleted User]","discy"):($anonymously_user != ""?esc_html__("Anonymous","discy"):get_comment_author($comment_id)));
                        	...
                        	<?php }
                        	do_action("discy_action_after_credential",(isset($post->ID)?$post->ID:0),$comment_id,$comment_user_id);?>
                    	</div>
                    	<a href="<?php echo get_comment_link($comment_id); ?>" class="comment-date"<?php echo(isset($its_question) && $its_question == "question"?" itemprop='url'":"")?>>
                    		<?php echo (is_single()?'<span itemprop="dateCreated" datetime="'.get_comment_date("F j, Y",$comment_id).'">':'');
                ...
                <div class="text">
                	...
                	<div<?php echo (isset($its_question) && $its_question == "question" && is_single()?" itemprop='text'":"")?>>
                		<?php if (isset($answer_args["comment_with_title"]) || isset($answer_args["comment_read_more"])) {
			            	echo '<p>'.wp_html_excerpt($comment->comment_content,apply_filters('discy_answer_number',300),'<a class="post-read-more comment-read-more" href="'.get_permalink($post_id).'#comment-'.esc_attr($comment_id).'" rel="bookmark" title="'.esc_attr__('Read more','discy').'">'.esc_html__('Read more','discy').'</a>').'</p>';
			            }else {
                	...
                	<?php if ($like_answer == true || (isset($its_question) && $its_question == "question")) {
                		if ($active_vote == "on" && $active_vote_answer == "on") {
                			if ($owner == false) {
                				$comment_vote_type = apply_filters("discy_comment_vote_type","comment",(isset($post->post_type)?$post->post_type:""));?>
	                	...
	                	<ul class="comment-reply comment-list-links">
	                		<li class="question-list-details comment-list-details">
								...
								<ul>
			                	    ...
			                	    if (class_exists("WPQA") && (($can_delete_comment == "on" && $comment_user_id == $user_get_current_user_id && $comment_user_id > 0 && $user_get_current_user_id > 0) || current_user_can('edit_comment',$comment_id) || is_super_admin($user_get_current_user_id))) {
...
if (!is_admin()) {
	function discy_sidebars($return = 'sidebar_dir') {
		...
		}else if (is_tag() || (is_archive() && !is_post_type_archive("question") && $tax_archive != true && !is_tax("question_tags"))) {
			$cat_sidebar_layout = discy_options("post_sidebar_layout");
		...
		}else if (is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || $tax_filter == true || $tax_archive == true || is_tax("question_tags") || is_post_type_archive("question")) {
...
function discy_ads($adv_type_meta,$adv_code_meta,$adv_href_meta,$adv_img_meta,$li = false,$page = false,$class = false,$author_cat = false,$question_columns = false) {
	...
	if ($author_cat == "on" && ((class_exists('WPQA') && wpqa_is_user_profile()) || is_category() || is_tax("question-category") || $tax_filter == true)) {
	...
	if ($author_cat == "on" && ((class_exists('WPQA') && wpqa_is_user_profile()) || is_category() || is_tax("question-category") || $tax_filter == true) && (($discy_adv_type == "display_code" && $discy_adv_code != "") || ($discy_adv_type == "custom_image" && $discy_adv_img != ""))) {
	...
	if (($adv_type == "display_code" && $adv_code != "" && $adv_code != "empty") || ($adv_type == "custom_image" && $adv_img != "")) {
		$output .= ($li == "li"?"":"<div class='clearfix'></div>").'
						...
						if ((isset($discy_home_tabs[$key]["sort"]) && isset($discy_home_tabs[$key]["value"]) && $discy_home_tabs[$key]["value"] != "" && $discy_home_tabs[$key]["value"] != "0") || (isset($discy_home_tabs[$key]["value"]) && isset($discy_home_tabs[$key]["cat"]))) {
							if (isset($discy_home_tabs[$key]["value"]) && isset($discy_home_tabs[$key]["cat"])) {
								...
								if ($last_url == "feed") {
									$last_url = $feed_slug;
									$class = "rss";
								}else if ($last_url == "recent-questions") {
									$last_url = $recent_questions_slug;
									$class = "book-open";
								}else if ($last_url == "questions-for-you") {
									$last_url = $questions_for_you_slug;
									$class = "book-open";
									...
									$class = "comment";
								}else if ($last_url == "answers-might-like") {
									$last_url = $answers_might_like_slug;
									$class = "comment";
								}else if ($last_url == "answers-for-you") {
									$last_url = $answers_for_you_slug;
									$class = "comment";
									...
									$class = "megaphone";
								}else if ($last_url == "followed") {
									$last_url = $question_followed_slug;
									$class = "plus-circled";
								}else if ($last_url == "favorites") {
									$last_url = $question_favorites_slug;
									$class = "star";
									...
									$class = "newspaper";
								}else if ($question_bump == "on" && $active_points == "on" && $last_url == "question-bump") {
									$last_url = $question_bump_slug;
									$class = "heart";
								}else if ($last_url == "feed-2") {
									$last_url = $feed_slug_2;
									$class = "rss";
								}else if ($last_url == "recent-questions-2") {
									$last_url = $recent_questions_slug_2;
									$class = "book-open";
								}else if ($last_url == "questions-for-you-2") {
									$last_url = $questions_for_you_slug_2;
									$class = "book-open";
									...
									$class = "comment";
								}else if ($last_url == "answers-might-like-2") {
									$last_url = $answers_might_like_slug_2;
									$class = "comment";
								}else if ($last_url == "answers-for-you-2") {
									$last_url = $answers_for_you_slug_2;
									$class = "comment";
									...
									$class = "megaphone";
								}else if ($last_url == "followed-2") {
									$last_url = $question_followed_slug_2;
									$class = "plus-circled";
								}else if ($last_url == "favorites-2") {
									$last_url = $question_favorites_slug_2;
									$class = "star";
									...
									$class = "newspaper";
								}else if ($question_bump == "on" && $active_points == "on" && $last_url == "question-bump-2") {
									$last_url = $question_bump_slug_2;
									$class = "heart";
								}
								do_action("discy_home_page_tabs_list",$last_url);
							}
							if ($tabs_menu != "" && $tabs_menu > 0) {
								$get_url = esc_url(add_query_arg("show",esc_attr($last_url),get_the_permalink($tabs_menu)));
							}else {
								$get_url = esc_url(add_query_arg("show",esc_attr($last_url),($page_template != ""?get_the_permalink($page_template):($category_id != ""?get_term_link($category_id,'question-category'):home_url('/')))));
							}?>
							
							<li<?php echo (isset($first_one) && $first_one != "" && $first_one == $last_url?" class='active-tab'":"")?>>
								<a href="<?php echo esc_url($get_url)?>">
									...
									if (isset($discy_home_tabs[$key]["sort"])) {
										if ($last_url == $feed_slug) {
											esc_html_e("Feed","discy");
										}else if ($last_url == $recent_questions_slug) {
											esc_html_e("Recent Questions","discy");
										}else if ($last_url == $questions_for_you_slug) {
										...
										}else if ($last_url == $answers_might_like_slug) {
											esc_html_e("Answers Might Like","discy");
										}else if ($last_url == $answers_for_you_slug) {
										...
										}else if ($last_url == $question_followed_slug) {
											esc_html_e("Followed Questions","discy");
										}else if ($last_url == $question_favorites_slug) {
										...
										}else if ($last_url == $answers_might_like_slug_2) {
											esc_html_e("Answers Might Like With Time","discy");
										}else if ($last_url == $answers_for_you_slug_2) {
										...
										}else if ($last_url == $question_followed_slug_2) {
											esc_html_e("Followed Questions With Time","discy");
										}else if ($last_url == $question_favorites_slug_2) {
										...
										do_action("discy_home_page_tabs_text",$last_url);
