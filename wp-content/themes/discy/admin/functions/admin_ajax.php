	...
	if(isset($all_save['import_setting']) && $all_save['import_setting'] != "") {
		...
		if (isset($all_save["roles"])) {$k = 0;
			foreach ($all_save["roles"] as $value_roles) {$k++;
				...
				if (isset($is_group)) {
					$roles_array = array("ask_question","show_question","add_answer","show_answer","add_post","add_category","send_message","upload_files","approve_question","approve_answer","approve_post","approve_comment","approve_question_media","approve_answer_media");
...
function discy_send_popup_notification() {
	if (class_exists("WPQA")) {
		echo $post_id = (int)$_POST["post_id"];
