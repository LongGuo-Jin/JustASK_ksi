...
function discy_options_fields($settings = array(),$option_name = "",$page = "options",$post_term,$options_arrgs = array()) {
	...
	foreach ( $options as $value ) {
		...
		if ($value['type'] != "heading") {
			...
			if ($value['type'] != "heading-2" && $value['type'] != "heading-3" && $value['type'] != "content" && $value['type'] != "info" && !isset($value['readonly'])) {
		...
		// Text input
		case 'text':
			$output .= '<input id="'.$field_id.'" class="discy-input discy-form-control" name="'.$field_name.'" type="text" value="' . esc_attr( $val ) . '"'.(isset($value['readonly'])?' readonly':'').'>';
			break;
		...
		case 'questions_categories':
			...
			<div class="styled-select">'.
				wp_dropdown_categories(array(
				    ...
				    'name'            => "",
				    'show_option_all' => (isset($value['show_option']) && $value['show_option'] != ""?$value['show_option']:esc_html__('Show All Categories','discy')),
				)).'
			</div>
			<div class="clear"></div>
			<div class="add-item add-item-2 add-item-6 add-item-7" data-toadd="'.(isset($value['toadd']) && $value['toadd'] != ""?$value['toadd']:"").'" data-addto="'.(isset($field_id) && $field_id != ""?$field_id:"").'" data-id="'.(isset($field_id) && $field_id != ""?$field_id:"").'_categories" data-name="'.(isset($field_id) && $field_id != ""?$field_id:"").'">'.esc_html__("Add category","discy").'</div>
			<div class="clear"></div>
		...
		case 'elements':
			...
			<input class="add_element'.($page == "meta" || $page == "terms" || $page == "author"?" no_theme_options":"").(isset($value['addto']) && $value['addto'] != ""?" add_element_to":"").'" type="button" value="'.(isset($value['button']) && $value['button'] != ""?$value['button']:esc_html__("+ Add new element","discy")).'"'.(isset($field_id) && $field_id != ""?" data-id='".$field_id."'":"").(isset($value['title']) && $value['title'] != ""?" data-title='".$value['title']."'":"").'>
			<div class="clear"></div>
			...
			if ($value['options'] != $val) {
				if (isset($val) && is_array($val)) {
					foreach ($val as $key_s => $key_s) {
						if (!isset($value['options'][$key_s]) && !isset($val[$key_s]["cat"]) && !isset($val[$key_s]["builder"])) {
			...
			foreach ($value_option as $key => $option) {
				...
				if (!isset($o_option["builder"])) {
					...
					$output .= '<li'.(isset($value['sort']) && $value['sort'] == "yes" && isset($val[$key]["cat"]) && $val[$key]["cat"] == "yes"?" class='categories' id='".$value['id']."_categories_".$val[$key]["value"]."'":'').'>';
						...
						if (isset($o_option["default"]) || (isset($val[$key]) && is_array($val[$key]) && array_key_exists('cat',$val[$key]))) {
