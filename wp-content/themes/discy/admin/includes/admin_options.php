	...
	function options_page() {?>
		<div id="discy-admin-wrap" class="discy-admin">
			...
			if ($max_input_vars < 3000) {
				echo '<div class="alert-message">'.sprintf(esc_html__('%1$s - Recommended Value: %2$s. Max input vars limitation will truncate POST data such as options. See: %3$sIncreasing max input vars limit.%4$s','discy'),$max_input_vars,"3000","<a href='https://2code.info/docs/discy/max_input_vars/' target='_blank' rel='noopener noreferrer'>","</a>").'</div>';
			}?>
		...
		if (isset($options) && is_array($options) && !empty($options)) {
			...
			foreach ( $options as $value ) {
				...
				if ( isset($value['type']) && $value['type'] == "heading" ) {
					...
					$menu .= '<a'.$template.' id="options-group-'.  $counter . '-tab" class="nav-tab ' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . (isset($value['icon']) && $value['icon'] != ''?'<span class="dashicons dashicons-'.esc_attr($value['icon']).'"></span>':'') . '<span class="options-name">'.esc_html( $value['name'] ).'</span></a>';
				}
			}
			return $menu;
		}
