<?php $show_the_content = apply_filters("discy_show_the_content",true,(isset($wp_page_template)?$wp_page_template:""));
...
if (empty($blog_h) && ((isset($tab_category) && $tab_category == true && ($first_one == $answers_slug || $first_one == $answers_slug_2))) || (isset($wp_page_template) && (($wp_page_template == "template-home.php" && is_user_logged_in() && isset($first_one) && $first_one != "" && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2))) || (isset($wp_page_template) && (($wp_page_template == "template-home.php" && isset($first_one) && $first_one != "" && ($first_one == $answers_slug || $first_one == $answers_slug_2 || (!is_user_logged_in() && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2)))) || $wp_page_template == "template-comments.php")))) {
	...
	if (isset($blog_h) && $blog_h == "blog_h") {
		...
		if ($loop_query == true) {
			...
			$show_sticky   = false;
			$post_not_true = false;
		}else if (((isset($tab_category) && $tab_category == true) || (isset($wp_page_template) && $wp_page_template == "template-question.php")) && isset($orderby_post) && ($orderby_post == "popular" || $orderby_post == "most_visited" || $orderby_post == "most_voted")) {
			$active_sticky = false;
			...
			if (($first_one == $feed_slug && is_user_logged_in()) || ($first_one == $feed_slug_2 && is_user_logged_in()) || (isset($wp_page_template) && $wp_page_template == "template-question.php" && $orderby_post == "feed" && is_user_logged_in())) {
				...
				if ($user_following_if == "yes" && $cat_following_if == "yes" && $tag_following_if == "yes" && ($all_following != "" || $user_following != "")) {
					...
					$query = $wpdb->prepare("SELECT SQL_CALC_FOUND_ROWS $wpdb->posts.* FROM $wpdb->posts 
						...
						".($specific_date_feed != ""?"AND ( $wpdb->posts.post_date >= '".$specific_date_feed."' ) ":"")."AND $wpdb->posts.post_type = 'question' AND ($wpdb->posts.post_status = 'publish' OR $wpdb->posts.post_status = 'private') GROUP BY $wpdb->posts.ID ORDER BY $wpdb->posts.post_date DESC LIMIT $post_number OFFSET $offset",1);
					$query = $wpdb->get_results($query);
					$total_query = $wpdb->get_var(apply_filters('found_posts_query','SELECT FOUND_ROWS()'));
				...
				$custom_sql = true;
			}else if (($first_one == $feed_slug && !is_user_logged_in()) || $first_one == $recent_questions_slug || ($first_one == $feed_slug_2 && !is_user_logged_in()) || ($wp_page_template == "template-question.php" && $orderby_post == "feed" && !is_user_logged_in()) || $first_one == $recent_questions_slug_2 || (!is_user_logged_in() && ($first_one == $questions_for_you_slug || $first_one == $questions_for_you_slug_2))) {
				$active_sticky        = true;
			...
			$array_data = apply_filters("discy_last_query_loop_page",array_merge($custom_category,$orderby_array,$cats_post,$specific_date_array,array("post_type" => $post_type,"ignore_sticky_posts" => 1,"paged" => $paged,"posts_per_page" => $post_number)),$first_one);
		}
	...
	}else if (class_exists('WPQA') && wpqa_is_pending_questions() && ($is_super_admin || $active_moderators == "on") && wpqa_is_user_owner() && ($is_super_admin || (isset($moderator_categories) && is_array($moderator_categories) && !empty($moderator_categories)))) {
		$array_data = array("post_status" => "draft","post_type" => "question","meta_query" => array($question_meta_query));
		if (!$is_super_admin && !in_array(0,$moderator_categories)) {
			$categories_question = array('tax_query' => array(array('taxonomy' => "question-category",'field' => 'id','terms' => $moderator_categories,'operator' => 'IN')));
			$array_data = array_merge($array_data,$categories_question);
		}
	}else {
