				...
				<div class="right-header float_r">
					...
					<?php }else {
						...
						<div class="user-login-area<?php echo ($active_notifications == "on"?"":" no-notifications")?>">
							<?php if (class_exists("WPQA") && $header_notifications == "on" && $active_notifications == "on" && $discoura_style != "on") {?>
							...
							<?php }
							if (class_exists("WPQA") && $header_messages == "on" && $active_message == "on" && $discoura_style != "on" && $header_style == "simple") {?>
							...
							if ($header_user_login == "on") {
								...
								if (class_exists("WPQA") && ($is_super_admin || $active_moderators == "on") && isset($user_login_links["pending-questions"]["value"]) && $user_login_links["pending-questions"]["value"] == "pending-questions") {
									...
									if ($is_super_admin || $user_moderator == "on") {
										...
										if ($is_super_admin || (is_array($moderator_categories) && !empty($moderator_categories))) {
											if ($is_super_admin || in_array(0,$moderator_categories)) {
												$num_pending = wpqa_count_posts_by_type("question","draft","no_asked");
											}else {
												$moderator_categories = implode(",",$moderator_categories);
												if ($moderator_categories != "") {
													$num_pending = wpqa_count_posts_by_user(0,"question","draft",$moderator_categories);
												}
											}
										}
									}
													...
													<?php }else if (isset($user_moderator) && ($is_super_admin || ($user_moderator == "on" && $active_moderators == "on")) && $key == "pending-questions" && isset($user_login_links["pending-questions"]["value"]) && $user_login_links["pending-questions"]["value"] == "pending-questions") {?>
														<li>
															<a href="<?php do_action("wpqa_get_profile",$user_id,"pending_questions")?>">
																<?php echo (isset($num_pending) && $num_pending != "" && $num_pending > 0?'<span class="notifications-number">'.wpqa_count_number($num_pending).'</span>':'')?>
					...
					if (class_exists("WPQA") && is_user_logged_in() && $header_style == "simple") {
						...
						if ($header_button == "question") {
							...
							$header_button_link = (class_exists("WPQA")?wpqa_add_question_permalink():"#");
							$header_button_text = esc_html__("Ask Question","discy");
						}else if ($header_button == "post") {
							$filter_class = "post";
							$header_button_class = "wpqa-post";
							$header_button_link = (class_exists("WPQA")?wpqa_add_post_permalink():"#");
							...
							<div class="mobile-bar-ask">
								<?php if (class_exists("WPQA")) {
									...
									if ($mobile_button == "question") {
										...
										$mobile_button_link = (class_exists("WPQA")?wpqa_add_question_permalink():"#");
										...
										$mobile_button_link = (class_exists("WPQA")?wpqa_add_post_permalink():"#");
			...
			<div class="discy-container">
				<main class="discy-main-wrap discy-site-content float_l">
					<div class="discy-main-inner float_l">
						...
						$update_profile = (is_user_logged_in() && class_exists("WPQA")?wpqa_update_profile($user_id):"");
						$cover_image = discy_options("cover_image");
						if (!is_home() && !is_front_page() && isset($breadcrumbs) && $breadcrumbs == "on" && $confirm_email != "yes" && $site_users_only != "yes" && ($cover_image != "on" || ($cover_image == "on" && class_exists("WPQA") && !wpqa_is_user_profile()) || ($cover_image == "on" && class_exists("WPQA") && (wpqa_is_user_edit_profile() || wpqa_is_user_delete_profile())))) {
