...
// With CSS You Can Modify the New Column Width – Here’s How
add_action('admin_head', 'kjl_custom_admin_css');
function kjl_custom_admin_css() {
  echo '<style>
  .column-num_posts {width: 8%}
  </style>';
}
...
function kjl_count_user_posts_by_type_and_status( $userid, $post_status, $post_type = "post" ) {
  ...
  function kjl_user_posts_count_column_content($value, $column_name, $user_id) {
	...
	if ( 'num_posts' == $column_name ) {
	  ...
	  return 'All: <a href="/wp-admin/edit.php?author='.$user_id.'" target="_blank">'.$all.'</a>
	  Draft: <a href="/wp-admin/edit.php?post_status=draft&amp;post_type=post&amp;author='.$user_id.'" target="_blank">'.$drafts.'</a>
	  Pending: <a href="/wp-admin/edit.php?post_status=pending&amp;post_type=post&amp;author='.$user_id.'" target="_blank">'.$pending.'</a>
	  Published: <a href="/wp-admin/edit.php?post_status=publish&amp;post_type=post&amp;author='.$user_id.'" target="_blank">'.$published.'</a>';
	}
	return $value;
  }

function modify_user_columns($column_headers) {
    $column_headers['full_name'] = 'Full Name';
    return $column_headers;
}
...
if (!function_exists('discy_body_classes')) {
	function discy_body_classes($classes) {
		...
		$classes[] = ($wp_page_template == "template-landing.php" || $under_construction == "on" || $site_users_only == "yes"?"discy_users_only":"discy_for_all");
...
if (!function_exists('discy_scripts_styles')) {
	function discy_scripts_styles() {
		...
		if (is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || $tax_filter == true || $tax_archive == true || is_tax("question_tags") || is_post_type_archive("question")) {
			if (is_category()) {
				$category_id = esc_attr(get_query_var('cat'));
			}else {
				$category_id = (int)get_query_var('wpqa_term_id');
			}
			if (is_category() || is_tag() || (is_archive() && !is_post_type_archive("question") && $tax_archive != true && !is_tax("question_tags"))) {
			...
			if (is_tag() || (is_archive() && !is_category() && !is_tax("question-category") && $tax_filter == true && !is_post_type_archive("question") && $tax_archive != true && !is_tax("question_tags"))) {
		...
		if ((is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || $tax_filter == true || $tax_archive == true || is_tax("question_tags") || is_post_type_archive("question")) && $primary_color_c == "") {
		...
		}else if ((is_category() || is_tag() || (is_archive() && !is_post_type_archive("question")) || is_tax("question-category") || $tax_filter == true || $tax_archive == true || is_tax("question_tags") || is_post_type_archive("question")) && $primary_color_c != "") {
		...
		if ($cover_image == "on" && class_exists('WPQA') && wpqa_is_user_profile() && !wpqa_is_user_edit_profile()) {
		...
		if (is_tax("question-category")) {
			...
			if (class_exists('WPQA') && $cat_cover == "on") {
		...
		if ($seo_active == "on") {
		    ...
		    if (!is_home() && !is_front_page() && (is_single() || is_page())) {
		    	...
		    	if (have_posts()) : while (have_posts()) : the_post();
	    			...
	    			$og_image = (function_exists("wpqa_image_for_share")?wpqa_image_for_share():"");
		    	    if (is_singular("question")) {
		    ...
		    }else if (class_exists('WPQA') && wpqa_is_user_profile() && !wpqa_is_user_edit_profile()) {
		    	$user_id = (int)get_query_var(apply_filters('wpqa_user_id','wpqa_user_id'));
		    	...
		    	$last_og_image = (!empty($fb_share_image)?$fb_share_image:($logo_display == "custom_image" && isset($logo_img) && $logo_img != ""?$logo_img:""));
...
if (!function_exists('discy_meta')) {
	function discy_meta($date = "",$category = "",$comment = "",$asked = "",$icons = "",$views = "",$post_id = 0) {
		...
		if ( $date == "on" ) {
			$time_string = '<time itemprop="datePublished" class="entry-date published" datetime="%1$s">%2$s</time>';
			...
			echo '<span class="post-date"'.('question' === $post_type?' itemprop="dateCreated"':'').' datetime="'.get_the_date("F j, Y",($post_id > 0?$post_id:"")).'">'.$posted_on.'</span>';
		}
		...
		if ('question' === $post_type) {
			...
			if ( $category == "on" ) {
				...
				$get_the_term_list = get_the_term_list(($post_id > 0?$post_id:get_the_ID()),'question-category',$first_span,', ',$second_span);
				if (!isset($get_the_term_list->errors) && $get_the_term_list != "") {
					...
					if (isset($term->slug)) {
						...
						echo ($first_span);
						echo esc_html($category_meta);
						echo ($second_span);
					}
				}
				do_action("discy_after_question_category",($post_id > 0?$post_id:get_the_ID()));
			}
		}
		if ( $comment == "on" && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			if ('question' === $post_type) {
				$num_comments = (int)get_comments_number();
				echo "<a href='".(get_the_permalink())."#comments'><span".(is_singular('question')?' itemprop="answerCount"':'')." class='number".($icons != "on"?" discy_hide":"")."'>".discy_count_number($num_comments).'</span>';
				if ($icons != "on") {
					echo " <span class='question-span'>".sprintf(_n("%s Answer","%s Answers",$num_comments,"discy"),$num_comments)."</span>";
				}
				echo "</a>";
			}else {?>
